...
<div class="container mx-auto px-4 py-8 max-w-6xl">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-blue-50 to-white">
            ...
            <p class="text-sm text-gray-600 mt-1">Modifiez les informations et le contenu du forfait</p>
        </div>

        <form action="{{ route('admin.elearning.forfaits.update', $forfait->id) }}" method="POST" class="p-6" id="forfaitForm">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    ...
                    <input type="text" name="name" id="name" required
                        ...
                        placeholder="Ex: Forfait Révision 30 jours" value="{{ old('name', $forfait->name) }}">
                    @error('name')
                ...
                <div>
                    ...
                    <input type="text" name="slug" id="slug" required
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                        placeholder="Ex: forfait-revision-30-jours" value="{{ old('slug', $forfait->slug) }}">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                <div>
                    ...
                    <input type="number" name="price" step="0.01" min="0" required
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                        placeholder="49.90" value="{{ old('price', $forfait->price) }}">
                    @error('price')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="duration_days" min="1" max="365" required
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                        placeholder="30" value="{{ old('duration_days', $forfait->duration_days) }}">
                    @error('duration_days')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="mb-8 p-4 bg-gray-50 rounded-lg border border-gray-200">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <label class="flex items-center">
                        <input type="checkbox" name="includes_qcm" value="1" {{ old('includes_qcm', $forfait->includes_qcm) ? 'checked' : '' }}
                            class="rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                        <span class="ml-2 text-sm text-gray-700">Inclut les QCM</span>
                    </label>
                    <label class="flex items-center">
                        <input type="checkbox" name="includes_examens_blancs" value="1" {{ old('includes_examens_blancs', $forfait->includes_examens_blancs) ? 'checked' : '' }}
                            class="rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                        <span class="ml-2 text-sm text-gray-700">Inclut les examens blancs</span>
                    </label>
                    <label class="flex items-center">
                        <input type="checkbox" name="includes_certification" value="1" {{ old('includes_certification', $forfait->includes_certification) ? 'checked' : '' }}
                            class="rounded border-gray-300 text-blue-600 focus:ring-blue-500">
            ...
            <div class="mb-8">
                ...
                @php
                    $isAllMode = $forfait->include_all_cours && $forfait->include_all_qcms && $forfait->include_all_examens;
                    $isCustomMode = !$isAllMode;
                @endphp
                    ...
                    <div class="flex flex-wrap items-center justify-between gap-4">
                        ...
                        <div class="flex space-x-6">
                            <label class="inline-flex items-center">
                                <input type="radio" name="selection_mode" value="all" {{ $isAllMode ? 'checked' : '' }}
                            ...
                            <label class="inline-flex items-center">
                                <input type="radio" name="selection_mode" value="custom" {{ $isCustomMode ? 'checked' : '' }}
                                       class="selection-mode-radio rounded-full border-gray-300 text-blue-600 focus:ring-blue-500">
                ...
                <div id="allModeSection" style="{{ $isAllMode ? 'display: block;' : 'display: none;' }}" class="space-y-4">
                        ...
                        <div class="flex items-start">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-green-700 mt-1">
                                    Tous les éléments actifs sont inclus dans ce forfait :
                                </p>
                ...
                <div id="customModeSection" style="{{ $isCustomMode ? 'display: block;' : 'display: none;' }}">
                    ...
                    <div class="mb-6 border border-gray-200 rounded-lg overflow-hidden shadow-sm">
                        ...
                        <div class="p-4 max-h-80 overflow-y-auto bg-white">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                                ...
                                @php
                                    $isChecked = $forfait->include_all_cours || in_array($coursItem->id, $forfait->selected_cours_ids ?? []);
                                    $isDisabled = $forfait->include_all_cours;
                                @endphp
                    ...
                    <div class="mb-6 border border-gray-200 rounded-lg overflow-hidden shadow-sm">
                                ...
                                <div class="flex items-center space-x-4">
                                    <label class="inline-flex items-center cursor-pointer">
                                        <input type="checkbox" name="include_all_qcms" value="1"
                                               ...
                                               {{ $forfait->include_all_qcms ? 'checked' : '' }}>
                        ...
                        <div class="p-4 max-h-80 overflow-y-auto bg-white">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                ...
                                @php
                                    $isChecked = $forfait->include_all_qcms || in_array($qcm->id, $forfait->selected_qcms_ids ?? []);
                                    $isDisabled = $forfait->include_all_qcms;
                                @endphp
                        ...
                        <div class="bg-gray-50 px-4 py-2 border-t border-gray-200 text-xs text-gray-500">
                            <span id="selectedQcmsCount">
                                @if($forfait->include_all_qcms)
                                    Tous
                                @else
                                    {{ count($forfait->selected_qcms_ids ?? []) }}
                                @endif
                            </span> QCM sélectionné(s)
                        </div>
                    </div>
                    ...
                    <div class="mb-6 border border-gray-200 rounded-lg overflow-hidden shadow-sm">
                                ...
                                <div class="flex items-center space-x-4">
                                    <label class="inline-flex items-center cursor-pointer">
                                        <input type="checkbox" name="include_all_examens" value="1"
                                               ...
                                               {{ $forfait->include_all_examens ? 'checked' : '' }}>
                                        <span class="ml-2 text-sm text-gray-600">
                        ...
                        <div class="p-4 max-h-80 overflow-y-auto bg-white">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                ...
                                @php
                                    $isChecked = $forfait->include_all_examens || in_array($examen->id, $forfait->selected_examens_ids ?? []);
                                    $isDisabled = $forfait->include_all_examens;
                                @endphp
                        ...
                        <div class="bg-gray-50 px-4 py-2 border-t border-gray-200 text-xs text-gray-500">
                            <span id="selectedExamensCount">
                                @if($forfait->include_all_examens)
                                    Tous
                                @else
                                    {{ count($forfait->selected_examens_ids ?? []) }}
                                @endif
                            </span> examen(s) blanc(s) sélectionné(s)
                        </div>
                    </div>
            ...
            <input type="hidden" name="include_all_cours" id="include_all_cours" value="{{ $forfait->include_all_cours ? '1' : '0' }}">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div class="flex items-center">
                    <label class="flex items-center">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $forfait->is_active) ? 'checked' : '' }}
                            class="rounded border-gray-300 text-blue-600 focus:ring-blue-500">
            ...
            <div class="mb-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
                <label class="flex items-center">
                    <input type="checkbox" name="create_stripe_product" value="1"
                        class="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                        {{ old('create_stripe_product') ? 'checked' : '' }}>
                ...
                @if($forfait->stripe_product_id)
                <p class="text-xs text-green-600 mt-2 ml-6">
                    <i class="fas fa-check-circle mr-1"></i> Produit Stripe déjà associé : {{ $forfait->stripe_product_id }}
                </p>
                @endif
            </div>
