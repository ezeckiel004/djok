...
@section('page-title', 'Modifier la session')
...
<div class="max-w-4xl mx-auto">
    <div class="bg-white shadow-sm rounded-lg border border-gray-200">
        <form action="{{ route('admin.sessions.update', $session) }}" method="POST" id="session-form">
            @csrf
            @method('PUT')
            ...
            <div class="px-4 py-5 sm:p-6 space-y-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="name" id="name" required
                                class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-djok-yellow focus:border-transparent transition-all duration-200"
                                value="{{ old('name', $session->name) }}" placeholder="Ex: Session Janvier 2024">
                            @error('name')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <select name="type" id="type" required
                                class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-djok-yellow focus:border-transparent transition-all duration-200">
                                <option value="presentiel" {{ old('type', $session->type) == 'presentiel' ? 'selected' : '' }}>Présentiel</option>
                                <option value="e_learning" {{ old('type', $session->type) == 'e_learning' ? 'selected' : '' }}>E-learning</option>
                                <option value="mixte" {{ old('type', $session->type) == 'mixte' ? 'selected' : '' }}>Mixte</option>
                            </select>
                            @error('type')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <input type="number" name="max_places" id="max_places" required min="1"
                                class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-djok-yellow focus:border-transparent transition-all duration-200"
                                value="{{ old('max_places', $session->max_places) }}">
                            @error('max_places')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                            <p class="mt-1 text-xs text-gray-500">Places disponibles actuelles : {{ $session->available_places }}</p>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="time" name="start_time" id="start_time"
                                class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-djok-yellow focus:border-transparent transition-all duration-200"
                                value="{{ old('start_time', $session->start_time ? $session->start_time->format('H:i') : '') }}">
                            @error('start_time')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="time" name="end_time" id="end_time"
                                class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-djok-yellow focus:border-transparent transition-all duration-200"
                                value="{{ old('end_time', $session->end_time ? $session->end_time->format('H:i') : '') }}">
                            @error('end_time')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="location" id="location"
                                class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-djok-yellow focus:border-transparent transition-all duration-200"
                                value="{{ old('location', $session->location) }}" placeholder="Adresse du centre de formation">
                            @error('location')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" step="0.01" name="price" id="price" min="0"
                                class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-djok-yellow focus:border-transparent transition-all duration-200"
                                value="{{ old('price', $session->price) }}" placeholder="Laisser vide pour utiliser le prix de la formation">
                            @error('price')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                ...
                <div class="bg-gray-50 rounded-lg p-6">
                    ...
                    <textarea name="description" id="description" rows="4"
                        ...
                        placeholder="Informations supplémentaires sur cette session...">{{ old('description', $session->description) }}</textarea>
                    @error('description')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="bg-gray-50 rounded-lg p-6">
                    ...
                    <div class="flex items-center p-4 bg-white rounded-lg border border-gray-200">
                        <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $session->is_active) ? 'checked' : '' }}
